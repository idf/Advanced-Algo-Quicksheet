...
Principle: minimal verbosity with minimal overloading and maximal readability. 
...
GS algorithm 1 dof (degree of freedom): which free man to propose in \pyinline{while} loop. After proof, it always produces the SAME stable matching. Thus, it is possible to always improve by lying.
...
\begin{figure}[!htp]
\centering
\subfloat{\includegraphics[scale=1.50]{4_2}}
...
\subsubsection{Improvement}
When $n\leq m$, each machine has at most 1 job; thus $T=t_{max}, \therefore T_\mat{G}=T_\mat{O}$.

When $n>m$, sort in \textbf{decreasing} order. \fbox{Improvement} from sorting. 
...
, where $j\in[1, m]$. Pigeon hole principle, assigned at leat two jobs.
\\

Consider max machine $i^*$, with the last assigned job $j^*$. Before assignment $j^*$ it is $T$

\begin{align*}
& T_\mat{G}=load(i^*) = T+t_{j^*}\\
& T_{i^*}-t_{j^*} \leq \bar T \text{ since greedy}\\
& T_{i^*}-t_{j^*} \leq T_\mat{O} \\
& t_{j^*} \leq t_{m+1} \text{ since index, sorted in desc order} \\
& t_{j^*} \leq \frac{1}{2}T_\mat{O}
\end{align*}

Regarding $t_j$, there are two cases:
\begin{enumerate}
\item If the max loaded machine only has 1 job, then $T = t_{max}$, which is trivial. 
...
Put the first center at the centering $s_{center}$, and then find the $s$ with max i.e. $s_t=\arg\max_s dist(s, C_t)$, put $c$ right at here. $t$ represents the current executing snapshot at time $t$. 
...
Proof by contradiction. Assume $\exists j, dist(s_j, C^k)\geq 2r_\mat{O}$. 

Distance between any two $s$ within a $r$-cover is at most $2r$. In $\mat{O}$, $C^*$. We can use the existence of this solution $C^∗$ in $\mat{O}$ though we do not know what $C^*$ is. Consider any site $s \in S$. There must be a center $c^* \in C^*$ that covers site $s$, and this center $c^*$ is at distance at most $r$ from $s$. $\mat{G}$ takes this $s$ as a center instead of $c^*$, as we have no idea what $c^*$ is. To make s cover all the sites that $c^*$ covers in the unknown solution $C^*$, this is accomplished by \fbox{expanding the radius} from $r$ to $2r$. All the sites that were at distance at most $r$ from center $c^*$ are at distance at most $2r$ from $s$ (by the triangle inequality in the circle).
...
\textbf{Vertex perspective}. Naive greedy: select $v\in V$ with the largest degree. Counter example that approximate ratio $>2$.

\textbf{Edge perspective}. Select the maximal matching from Network Flow. Collection of edges that do not share $v$.
...
Graph for price allocation: 

\begin{figure}[!htp]
\centering
\subfloat{\includegraphics[scale=1.40]{11_8}}
\caption{Pricing Method for Vertex Cover}
\label{fig:11_8}
\end{figure}
...
Primal dual method: assign vertex \textbf{costs} to edge as the \textbf{price}. 

\textbf{Fair pricing}The price must be fair. 
...
\end{figure}
\begin{itemize}
\item Each set $S_i$ with weights (i.e. price) $w_i\geq 0$. 
...
, where $R$ is the remaining vertexes to be covered. 
...
When each vertex $v$ is purchased, $v$ pays an \textbf{amortized price}. 
...
\caption{Harmonic function and difference $\gamma$}
\label{fig:gammaArea}
\end{figure}
